...
function linterTasks(gulp, opts) {
  ...
  gulp.task('jslint', function () {
    ...
    var jslintPipe = (0, _gulpEslint2.default)(eslintRules);

    if (options.lintCache) {
      (function () {
        var eslintRuleSet = _fs2.default.readFileSync(eslintRules.configFile, 'utf8');

        jslintPipe = (0, _gulpCache2.default)(jslintPipe, {
          key: function key(file) {
            return eslintRuleSet + file.contents.toString('utf8');
          },
    ...
    return gulp.src([].concat(options.jsAssets || []).concat(options.testPaths || [])).pipe(jslintPipe).pipe(_gulpEslint2.default.formatEach()).pipe(_gulpEslint2.default.failAfterError()).on('error', function () {
      return process.exit(1);
    });
  });
};

var _default = linterTasks;
...
var _temp = function () {
  ...
  __REACT_HOT_LOADER__.register(linterTasks, 'linterTasks', 'src/gulp-tasks-linters.js');

  __REACT_HOT_LOADER__.register(_default, 'default', 'src/gulp-tasks-linters.js');
}();
